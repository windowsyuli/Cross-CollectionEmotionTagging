...
namespace SharpICTCLAS
{
   public class WordSegment
   {
      ...
      public List<WordResult[]> Segment(string sentence)
      {
       ...
       public Hashtable Segmen_Test()
       {
           Hashtable result = new Hashtable();
           foreach(IndexTableItem table in m_dictCore.indexTable)
               if (table.WordItems!=null)
           {
               foreach (WordItem wt in table.WordItems)
               {
                   string w = wt.sWord;
                   if (w!= "")
                   {
